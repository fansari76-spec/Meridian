...
export function useItinerary() {
  ...
  const generate = useCallback(async ({ destination, days, interests, cuisine, faithTradition }) => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/api/itinerary/generate`, {
        ...
        body: JSON.stringify({ destination, days, interests, cuisine, faithTradition }),
      });
